...
public class UDPReceiver extends Thread {
	...
	private int NICE_QR_MASK = 0x80;
	private int NICE_OPCODE_MASK = 0x78;
	...
	public void run() {
		try {
			...
			while (!this.stop) {
				...
				byte[] bytes = new byte[TabInputStream.available()];
				TabInputStream.read(bytes,0,bytes.length);

				int id = (int)bytes[0] << 8 | bytes[1];
				int qr = (int)(bytes[2] & NICE_QR_MASK) >>> Integer.numberOfTrailingZeros(NICE_QR_MASK);
				int opCode = (int)(bytes[2] & NICE_OPCODE_MASK) >>> Integer.numberOfTrailingZeros(NICE_OPCODE_MASK);
				int anCount = (int)bytes[6] << 8 | bytes[7];
				//int qType = (int)bytes[6] << 8 | bytes[7];
				//int qClass = (int)bytes[6] << 8 | bytes[7];
...
//			serveur.close(); //closing server
	...
	private String getQName(byte[] bytes){
		int nbDeBytes;
		int cursor = 12;
		String qName = "";
		nbDeBytes = (int)bytes[cursor];
		while (nbDeBytes != 0){
			System.out.println(nbDeBytes);
			for (int i=cursor+1; i<cursor+1 + nbDeBytes; i++){
				char[] chars = Character.toChars((int) bytes[i]);
				qName += Character.toString(chars[0]);
			}
			qName += ".";
			cursor += nbDeBytes+1;
			nbDeBytes = (int)bytes[cursor];
